...
export default function ShoppingCalendarScreen() {
    ...
    // 💡 6개 행이 남은 공간에 딱 맞춰지도록 동적 셀 높이 계산 (최소 높이 50 보장)
    const calculatedCellHeight =
        calendarHeight > 0 ? Math.floor((calendarHeight - ROW_GAP * 5) / 6) : 60;
    const cellHeight = Math.max(calculatedCellHeight, 50);
    ...
    const WEEKDAYS = [
        ...
        { label: "mon", textClass: "text-text-default", bgClass: "bg-bg-paper border-divider" },
        { label: "tue", textClass: "text-text-default", bgClass: "bg-bg-paper border-divider" },
        { label: "wed", textClass: "text-text-default", bgClass: "bg-bg-paper border-divider" },
        { label: "thu", textClass: "text-text-default", bgClass: "bg-bg-paper border-divider" },
        { label: "fri", textClass: "text-text-default", bgClass: "bg-bg-paper border-divider" },
        {
            label: "sat",
            textClass: "text-text-default",
    ...
    return (
        <View className="flex-1 bg-bg-default py-2 mb-4">
            ...
            {/* 3. 달력 그리드 영역 (onLayout으로 남은 높이를 측정하여 화면에 꽉 차게 조절) */}
            <View
